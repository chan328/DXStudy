...
bool SystemClass::Initialize()
{
	...
	InitializeWindows(screenWidht, screenHeight);
	...
	// graphics 객체를 초기화합니다.
	result = m_Graphics->Initialize(screenWidht, screenHeight, m_hwnd);
	if (!result)
	{
		return false;
	}
...
LRESULT CALLBACK SystemClass::MessageHandler(HWND hwnd, UINT umsg, WPARAM wparam, LPARAM lparam)
{
	switch (umsg)
	{
		...
		case WM_KEYUP:
		{
			// 키가 떼어졌다면 input 객체에 이 사실을 전달하여 이 키를 해체토록합니다.
...
void SystemClass::InitializeWindows(int& screenWidth, int& screenHeight)
{
	...
	if (FULL_SCREEN)
	{
		...
		// 윈도우의 위치를 화면의 외쪽 위로 맞춥니다.
		posX = posY = 0;
	}
	else
	{
		// 윈도우 모드라면 800 x 600의 크기를 가지게 합니다.
		screenWidth = 800;
		screenHeight = 600;
